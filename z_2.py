...
def biba_boba(n, x):
    return (x * sp.spherical_jn(n - 1, x) - n * sp.spherical_jn(n, x)) / (x * flopa(n - 1, x) - n * flopa(n, x))


def shlepa(n, x):
    return sp.spherical_jn(n, x) / flopa(n, x)


massivSum = []
for sigN in range(1, 20):
    massivSum.append(((-1) ** sigN) * (sigN + 0.5) * (biba_boba(sigN, k * r) - shlepa(sigN, k * r)))
